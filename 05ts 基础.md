...
1. 全面拥抱了类型推导 
2. 基于已有的类型，通过编程的手段，进行转化加工，最终得到一个新类型
3. 解决了ide/编辑器不能智能提示的痛点
4. 解决了，其它开发者偷偷修改了字段类型/字段个数，而导致项目出现问题。
5. 提供的编译期类型检查可以解决类型报错
6. 写代码忘记细节的痛点，解决了写 js 时候不敢轻易修改不合理的接口的痛点
7. 生成文档 

无法知道传入的参数类型，需要string传入null，对象等会被强转为string，不能判断执行结果是什么

代码库一旦庞大起来，强类型非常有价值，当一个函数 caller 和 callee 分别由不同团队的不同人编写时，只有强类型接口是双方的契约，谁违反了契约静态分析能够立即发现。这对于重构来说也非常有价值，重构过程中如果契约被违反了，静态分析马上能知道，不需要等跑测试，更不会等上线后才发现。 

在本质上都是**要求在程序不运行的情况下对程序的行为进行理解**，而数据类型的意义就在于提供了这样的手段
...
之前开发过程中蹭蹭写完代码一个 reload 后发现 devtool 几行红字

Cannot read property 'xxx' on undefined

'undefined' is not a function

第一，字段不是固定的，是根据一个 schema 渲染出来的。那个 schema 本身也是有编辑界面的，不是一个表单，是好多表单组合成一个应用。而且 schema 是树型的，导致通过 schema 渲染出来的是字段套字段的复杂表单。

第二，字段类型多，每种类型都有个性化的渲染方式。不是开源库里那种 booleanfiled，textfield 能解决的。

比如一个原本很简单的数字框，要支持宏语法，类似$TIMEATAMP - 1000这种，还要判断模板语法对不对，输入$后还要下拉提醒宏变量列表，自动完成。工作量直线上升。

第三，本来就很复杂的东西，还要支持复制粘贴，撤销重做，预览等等一大堆编辑功能。最变态的是还要支持三种模式：编辑模式/调试模式/运行模式。界面看起来差不多，但是实际开发起来，要改的地方特别多，复用很难，你觉得你能靠设计模式解决，真做起来掉到坑里就出不来。
